...
function EmissaoCertificadoModal({ isOpen, onClose, onConfirm, aluno, loading, actionType, allStudents, courses }) {
  ...
  const buildAutoTextForForm = (form = {}) => buildCertificatePreviewText({
    nome: aluno?.nome,
    cpf: aluno?.cpf,
    nomeCurso: form?.cursoNome,
    ...
    setCourseForms((prev) => prev.map((item) => (
      item.cursoId === current.cursoId
        ? (() => {
            ...
            const prevAutoText = buildAutoTextForForm(item);
            const prevText = String(item.textoCertificado || '').trim();
            return {
              ...item,
              [field]: value,
              textoCertificado: ['local', 'data', 'duracao', 'periodoInicio', 'periodoFim'].includes(field)
                ? (
                    !prevText || prevText === prevAutoText
                      ? buildAutoTextForForm({
                          ...item,
                          [field]: value,
                        })
